...
async function run() {
  try {
    ...
    app.put("/userPremium/:email", async (req, res) => {
      const {time} = req.body;
      const {expiryDate} = await userCollection.findOne({email:req.params.email})
      if(time>expiryDate){
    ...
    app.get("/articles",async(req,res)=>{
      ...
      const skip = (page - 1) * limit;
      const result = await articleCollection.find(query).sort({createAt:-1}).skip(skip).limit(limit).toArray();
    ...
    app.get("/article",async(req,res)=>{
      const result = await articleCollection.find({status:'approved'})
      .sort({ viewCount: -1 })
      .limit(6).toArray();
      res.send(result)
    })
